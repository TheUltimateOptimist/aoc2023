...
void print_mappings(unordered_map<string, Mapping>& mappings) {
    for (const auto& pair : mappings) {
        cout << pair.first << endl;
        for (const auto& operand : pair.second.operands) {
            cout << operand << endl;
        }
        for (const auto& operation: operations) {
            if (operation.second == pair.second.operation) {
...
std::uint16_t evaluate(unordered_map<string, Mapping>& mappings, const string& wire, int depth) {
    std::cout << depth << " " << wire << std::endl;
    ...
    for (const auto& operand : mapping.operands) {
        if (operand[0] >= '0' && operand[0] <= '9') {
            ...
            if (some == cache.end()) {
                uint16_t value = evaluate(mappings, operand, depth + 1);
                cache[operand] = value;
...
int main(int argc, char** argv) {
    ...
    cout << (evaluate(mappings, "a", 0)) << endl;
    return 0;
}