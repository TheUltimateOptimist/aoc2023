...
def do(direction: Direction, some: Callable[[Direction], Direction]) -> Direction:
    return some(direction)

direction = Direction(0, 1)
do(direction, Direction.rotatea)
...
class Runner[T]:
    ...
    def next(self) -> Point | None:
        ...
        for redirecter in self.redirecters:
            while grid.get(next) == redirecter.on:
                self.direction = redirecter.next(self.direction)
                next = next.next(self.direction)
    ...
    def get_prev(self) -> T:
        return grid.get(self.previous)
        
@dataclass(frozen=True)
class Node:
    x: Direction | None = None
    s: int = -1
    s_d: Direction | None = None
...
for i in range(len(grid.data)):
    mem = []
    for j in range(len(grid.data[0])):
        mem.append(Node())
    memory_list.append(mem)
memory = Grid[Node](memory_list)

while runner.next():
    if runner.direction != direction:
        continue
    index+=1
    has_found = False
    loop_runner = Runner(
        runner.grid, 
        runner.redirecters, 
        runner.redirecters[0].next(runner.direction), 
        runner.previous
    )
    while loop_runner.next():
        if loop_runner.get().
