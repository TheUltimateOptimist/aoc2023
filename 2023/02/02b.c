...
int gamePow(char * line, int beginning, int maxRed, int maxGreen, int maxBlue) {
    int minRed = 1;
    int minGreen = 1;
    int minBlue = 1;
    char number[5];
    ...
    for (int i = beginning; i < strlen(line); i++) {
        ...
        if ((line[i] == 'r' || line[i] == 'g' || line[i] == 'b') && numberIndex > 0) {
            ...
            switch (line[i]) {
                case 'r': {
                    if (numberValue > minRed) {
                        minRed = numberValue;
                    }
                };
                break;
                case 'g': {
                    if (numberValue > minGreen) {
                        minGreen = numberValue;
                    }
                }
                break;
                case 'b': {
                    if (numberValue > minBlue) {
                        minBlue = numberValue;
                    }
                }
            }
...
int main() {
    ...
    for (int i = 1; i <= 100; i++) {
        ...
        sum += gamePow(line, 7 + numberOfDigits(i), maxRed, maxGreen, maxBlue);
    }
    printf("%d\n", sum);
}