...
def get_conf(direction: tuple[int, int], row: int, col: int) -> int:
    visited.clear()
    seen.clear()
    go(direction, row, col)
    return len(visited)

max_conf = 0
for i in range(len(grid)):
    max_conf = max(max_conf, get_conf(right, i, 0), get_conf(left, i, len(grid[0]) - 1))
for i in range(len(grid[0])):
    max_conf = max(max_conf, get_conf(down, 0, i), get_conf(up, len(grid) - 1, i))

print(f"result = {max_conf}")